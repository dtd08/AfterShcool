...
- BadObjectPrinter는 구체적인 타입인 Car, Dog를 사용한다.
  따라서 이후에 출력해야할 구체적인 클래스가 10개로 늘어나면
  클래스에 맞추어 메서드도 10개로 늘어나야 한다.
- 이렇게 BadObjectPrinter 클래스가 구체적인 특정 클래스인 Car, Dog를 사용하는 것을
  BadObjectPrinter는 Car, Dog에 의존한다고 표현한다.
- 이 문제를 해결하기 위해 객체의 정보를 사용할 때 다형적 참조 문제를 해결해줄 Object 클래스와
  메서드 오버라이딩 문제를 해결해 줄 Object.toString() 메서드가 있다.
